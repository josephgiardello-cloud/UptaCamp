...
class VoiceManager:
    ...
    def speak_bert(
        ...
        if not bypass_cooldown:
            if (now - self._last_spoken_at) < self.min_interval_s:
                return
    ...
    def _shape_for_voice_style(self, text: str, voice_style: str) -> str:
        ...
        for pattern, repl in replacements.items():
            shaped = re.sub(pattern, repl, shaped, flags=re.IGNORECASE)
        return shaped
    ...
    def _speak_local_ai(self, text: str, dad_ai_level: int) -> None:
        ...
        if shutil_which(exe) is None and not Path(exe).exists():
            if self._is_windows:
                self._speak_windows(text, dad_ai_level)
            return

        cache_key = self._build_cache_key(text, dad_ai_level, str(model_path))
    ...
    def _build_cache_key(self, text: str, dad_ai_level: int, model_path: str) -> str:
        raw = f"{text}|{dad_ai_level}|{self.backend}|{model_path}"
    ...
    def _speak_windows(self, text: str, dad_ai_level: int) -> None:
        if dad_ai_level == 4:
            ...
            preferred_voices = ["Microsoft Zira Desktop", "Microsoft Aria"]
            rate = 1
    ...
    def get_local_prerequisites_report(self) -> dict[str, Any]:
        report: dict[str, Any] = {
            ...
            "recommended": {
                ...
                "bert_plus_level5": ["Microsoft Zira Desktop", "Microsoft Aria"],
            },
            "notes": [],
        }

        exe = self.local_ai_exe_path or "piper"
        exe_found = shutil_which(exe) is not None or Path(exe).exists()
        ...
        rvc_exe_found = shutil_which(rvc_exe) is not None or Path(rvc_exe).exists()
